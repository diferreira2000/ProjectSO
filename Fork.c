...
void handler(int signum)
{
    if (signum == SIGUSR1)
    perc++;
    double resu = ((double)perc/nr_gen)*100;
    printf("perc:%d || nr_gen:%lf",perc,nr_gen);
    printf("(%3.2lf%%)\e[u", resu);
    fflush(stdout);
...
#define TAMSG 100
    ...
    GENOME *g = gl->phead;
    MUTATION *ma = mutation_array->mutations;
    ...
    for (int i = 0; i < nr_filhos; ++i)
    {
        ...
        sprintf(res, "result/mutations%d.txt", i + 1); //dar nomes aos ficheiros
        if ((pids[i] = fork()) < 0)
        {
            perror("fork");
            exit(1);
        ...
        else if (pids[i] == 0)
        {
            ...
            while (g != NULL)
            {
                ...
                kill(getppid(), SIGUSR1); //mandamos o sinal quando um filho acaba as comparaçoes todas
...
void save_mutation_array(MUTATION_ARRAY *mutation_array, char *path, int detail, int pid)
{
    ...
    for (i = 0; i < mutation_array->n_mutations; i++)
    {
        ...
        sprintf(buf, "%d|%s;%s|%s|(%d)", pid, aux->genome_a, aux->genome_b, aux->gene, aux->seq_mutations.n);
        if (detail)
        {
            int j;
