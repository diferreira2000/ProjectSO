...
#define TAMSG 100
    ...
    MUTATION *ma = mutation_array->mutations;
     if (pipe(fds) < 0)exit(-1); //cria o pipe
       ...
       if (i == nr_filhos)
        ...
        else if (pids[i] == 0)
        {
            ...
            while (g != NULL)
            {
                ...
                save_mutation_array_pipe(mutation_array, res, 0,getpid(),fds[1]);
    ...
    int fd = open(res1, O_WRONLY | O_APPEND | O_CREAT, 0744);
    if (fd == -1)
    {
        perror("File open");
        exit(1);
    }
    while (read(fds[0], tmp, sizeof(tmp)) > 0)//definir macro para o size of
    {
